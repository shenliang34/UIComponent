    ...
    void ResetChildPosition()
    {
        ...
        for (i = 0; i < imax; i++)
        {
            ...
            else
            {
                ...
                cullContent = false;//如果预制Item数超过maxIndex则将超过部分隐藏 并 设置cullCintent为ufalse 并且不再更新 panel尺寸
            }
    ...
    void WrapContent()
    {
        ...
        if (mHorizontal)
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos=temp.localPosition;
                    bool isActive = (pos.x > min && pos.x < max) ? true : false;
        ...
        else 
        {
            ...
            for (int i = 0, imax = mChild.Count; i < imax; i++)
            {
                ...
                {
                    Vector2 pos = temp.localPosition;
                    bool isActive = (pos.y > min && pos.y < max) ? true : false;
                    temp.gameObject.SetActive(isActive);
                }
